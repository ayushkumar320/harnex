...
if TYPE_CHECKING:
    from agentharness.scan_models import ExcludedPath, StructuralFact
...
CATALOG: dict[str, CatalogRule] = {
    ...
    "AH-U101": CatalogRule(
        ...
        impact="AgentHarness may miss providers, tools, or entry points hidden behind lookup.",
...
def load_suppressions(root: Path) -> list[Suppression]:
    path = root / ".agentharness" / "suppressions.yml"
    if not path.exists():
        return []
