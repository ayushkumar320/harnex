...
def verify_repository(
    ...
    *,
    ...
    workspace_parent = Path(tempfile.mkdtemp(prefix="agentharness-verify-"))
...
def render_verification_summary(
    ...
    *,
    ...
    console.print("AgentHarness verify uses fixture credentials and denied-network sandbox checks.")
...
def _check_scan_artifact_current(workspace: Path) -> VerificationCheck:
    def run() -> VerificationCheck:
        scan_path = workspace / ".agentharness" / "scan.json"
        if not scan_path.exists():
            return VerificationCheck(
                ...
                status=NOT_EXERCISED,
                component="scan",
                evidence=[".agentharness/scan.json was not present in the disposable workspace."],
...
def _check_generated_python_importable(workspace: Path) -> VerificationCheck:
    def run() -> VerificationCheck:
        generated = workspace / ".agentharness" / "generated"
        if not generated.exists():
            return VerificationCheck(
                ...
                evidence=["No generated AgentHarness Python files were present."],
...
def _check_runtime_retry_and_redaction(workspace: Path) -> VerificationCheck:
    def run() -> VerificationCheck:
        ...
        log_path = workspace / ".agentharness" / "verify-runtime.jsonl"
...
def _check_sandbox_containment(
    ...
    def run() -> VerificationCheck:
        ...
        if probe.status is not SandboxCapabilityStatus.SUPPORTED:
            # An unavailable sandbox means containment was never tested, not that it failed.
            return VerificationCheck(
