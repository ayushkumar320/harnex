...
from agentharness.errors import ConfigurationError, ErrorContext
from agentharness.external_evidence import WebEvidenceConfig
from agentharness.output import ColorMode, OutputFormat
...
ENV_MAP = {
    "output_format": "AGENTHARNESS_OUTPUT_FORMAT",
    "color": "AGENTHARNESS_COLOR",
    "log_level": "AGENTHARNESS_LOG_LEVEL",
    "telemetry_enabled": "AGENTHARNESS_TELEMETRY_ENABLED",
    "model_provider": "AGENTHARNESS_MODEL_PROVIDER",
}
...
def load_config(overrides: ConfigOverrides | None = None) -> AppConfig:
    ...
    if "AGENTHARNESS_WEB_EVIDENCE_ENABLED" in os.environ:
        web = _mapping_value(values["web_evidence"].value)
        web["enabled"] = _coerce_env_value(
            "web_evidence.enabled",
            os.environ["AGENTHARNESS_WEB_EVIDENCE_ENABLED"],
        )
        values["web_evidence"] = ConfigValue(value=web, source=Source.ENVIRONMENT)
    if "AGENTHARNESS_TAVILY_MAX_CREDITS" in os.environ:
        web = _mapping_value(values["web_evidence"].value)
        web["max_credits_per_command"] = int(os.environ["AGENTHARNESS_TAVILY_MAX_CREDITS"])
        values["web_evidence"] = ConfigValue(value=web, source=Source.ENVIRONMENT)
...
def _default_config_path() -> Path | None:
    for candidate in (Path("agentharness.yaml"), Path("agentharness.yml")):
        if candidate.exists():
            return candidate
    return None
...
def _load_config_file(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise ConfigurationError(
            ...
            context=ErrorContext(
                field="config_path",
                source="config file",
                expected="Valid YAML with AgentHarness settings.",
    ...
    section = document.get("agentharness", document)
    if not isinstance(section, Mapping):
        raise ConfigurationError(
            ...
            context=ErrorContext(
                ...
                source="config file",
                expected="YAML key-value settings.",
                next_action="Make the agentharness section a mapping.",
            ),
        )
    return dict(section)
        ...
        return RouterConfig(
            ...
            route=[
                RouteEntry(
                    ...
                    model=os.environ.get("AGENTHARNESS_GROQ_MODEL", "llama-3.1-8b-instant"),
                    locality=ProviderLocality.REMOTE,
                )
            ],
        )
    if normalized == ModelProvider.HUGGINGFACE:
        model = os.environ.get("AGENTHARNESS_HF_MODEL", "meta-llama/Llama-3.1-8B-Instruct")
        return RouterConfig(
            enabled=True,
            data_policy=DataPolicy.REMOTE_ALLOWED,
            route=[
        ...
        return RouterConfig(
            ...
            route=[
                RouteEntry(
                    ...
                    model=os.environ.get("AGENTHARNESS_OPENAI_COMPATIBLE_MODEL", "local-model"),
...
def _expected_for(field: str) -> str:
    ...
    return expected.get(field, "a supported AgentHarness setting")
