...
SUPPORTED_OUTPUT_FILES = GENERATED_OUTPUT_FILES
    ...
    try:
        ...
        raise AgentHarnessError(
            ...
            context=ErrorContext(
                field="plan",
                source="argument",
                expected="A completed AgentHarness harness_plan JSON artifact.",
    ...
    try:
        ...
        raise AgentHarnessError(
            ...
            context=ErrorContext(
                ...
                expected="A readable AgentHarness apply_transaction_journal artifact.",
...
def stage_apply_preview(
    *,
    ...
    staging_root = repository_root / ".agentharness" / "staging" / _hash_suffix(plan_hash)
...
def apply_approved_plan(
    *,
    ...
    journal_path = repository_root / ".agentharness" / "transactions" / f"{transaction_id}.json"
...
def render_apply_preview(console: Console, preview: ApplyPreview, *, artifact_path: Path) -> None:
    if preview.mode == "dry_run":
        console.print(
            "AgentHarness apply dry-run: staged files were written under .agentharness only."
        )
    else:
        console.print("AgentHarness apply completed with a transaction journal.")
...
def _validate_plan_source(plan: HarnessPlan) -> Path:
    ...
    from agentharness.planning import build_plan
...
def _atomic_write(path: Path, data: bytes) -> None:
    temp = path.with_name(f".{path.name}.agentharness-tmp")
    try:
...
def _reapply_conflict(path: str, reason: str) -> AgentHarnessError:
...
def _render_templates(action: PlanAction) -> dict[str, str]:
    ...
    return {
        ".agentharness/generated/agentharness_config.py": (
            '"""Generated AgentHarness runtime configuration.\n\n'
            ...
            'DEFAULT_RUNTIME_LOG_PATH = ".agentharness/runtime/runtime.jsonl"\n'
        ...
        ".agentharness/generated/agentharness_jsonl_logger.py": (
            '"""Generated JSONL logging helpers for AgentHarness runtime evidence."""\n\n'
            "from __future__ import annotations\n\n"
            "import json\n"
            "from pathlib import Path\n\n\n"
            "from agentharness.runtime import RuntimeEvent, RuntimeJsonlWriter, "
        ...
        ".agentharness/generated/agentharness_runner.py": (
            ...
            ")\n\n"
            "from agentharness_config import (\n"
            "    ADAPTER,\n"
            "    DEFAULT_RETRY_POLICY,\n"
        ...
        ".agentharness/generated/tests/test_agentharness_smoke.py": (
            '"""Generated smoke tests for the staged AgentHarness runtime wrapper."""\n\n'
            "from pathlib import Path\n\n"
            "from agentharness.runtime import RuntimeFailure, RuntimeFailureKind\n"
            "from agentharness_runner import describe_generated_harness, run_direct_provider\n\n\n"
            "class FakeClock:\n"
...
def _path_error(path: str) -> AgentHarnessError:
    return AgentHarnessError(
        code="AH-G006",
...
def approve_plan_actions(plan_path: Path) -> tuple[HarnessPlan, list[str], list[str]]:
    """Mark supported plan actions approved in place; return the plan and action ids.

    Deterministic scanning cannot approve its own repairs, so a plan artifact always
    arrives with `unresolved` actions. This records an explicit human approval for the
    actions that already satisfy every generation constraint and leaves the rest alone.
    """
    plan, _plan_hash = load_plan_artifact(plan_path)
    approved: list[str] = []
    skipped: list[str] = []
    actions: list[PlanAction] = []
    for action in plan.actions:
        if action.approval_state == "approved":
            ...
            continue
        candidate = action.model_copy(update={"approval_state": "approved"})
        if action.approval_state == "unresolved" and not _action_issues(candidate):
    ...
    if not approved:
        raise AgentHarnessError(
            code="AH-G013",
            message="Plan contains no action that can be approved for generation.",
            context=ErrorContext(
                field="actions",
                source="plan artifact",
                expected="At least one unresolved action matching a supported generation template.",
                next_action="Review the plan; the reported findings need a manual repair.",
            ),
            exit_code=4,
        )
    updated = plan.model_copy(update={"actions": actions})
    plan_path.write_text(canonical_plan_json(updated) + "\n", encoding="utf-8")
